...
/// both strings that can be parsed by `T::from_str`. `seperator` must be an ASCI character.
/// 
...
/// Parse a pair of floating-point numbers seperated by a comma as complex  number
/// 
...
///
/// `bounds` is a pair giving the width height of the image in pixels.
...
/// arguments specify points on the complex plane corresponding to the upper-left and lower-right corners of the pixel buffer.
/// 
...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<i32>("100x50",'x'), Some((100,50)));
    assert_eq!(parse_pair::<u32>("10&10", '&'), Some((10,10)));
